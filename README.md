...
- **배포 URL (GitHub Pages)**: `https://jih19984.github.io/Codyssey-AI-SW-Basic-B1-1/` _(배포 후 업데이트)_
- **작성자**: 정인호 (`jih19984`)
...
### 프로젝트 기본 구성
- [ ] `index.html`, `css/`, `js/`, `images/` 폴더 구조 분리
- [ ] 외부 스타일시트/스크립트 HTML 연결
- [ ] VS Code + Live Server 개발 환경 구성

### HTML 구조 (시맨틱 마크업)
- [ ] `<header>`, `<nav>`, `<main>`, `<section>`, `<article>`, `<footer>` 사용
- [ ] Hero / About / Skills / Projects / Contact / Footer 섹션 포함
- [ ] 네비게이션에 각 섹션 앵커 링크
- [ ] 모든 이미지에 의미있는 `alt` 속성
- [ ] 폼 `<label for>` - `id` 매칭

### CSS 스타일링
- [ ] `css/style.css` 외부 스타일시트 사용
- [ ] `:root` CSS 변수 (색상/폰트/간격)
- [ ] `[data-theme="dark"]` 다크모드 변수
- [ ] 네비게이션 Flexbox 레이아웃
- [ ] Projects 카드 Grid (`auto-fit`, `minmax`) 반응형
- [ ] 모바일 퍼스트 + 768px/1024px 브레이크포인트
- [ ] 모바일에서 네비 숨김 + 햄버거 버튼 노출
- [ ] 버튼/카드 hover + transition, box-shadow

### JavaScript 기초
- [ ] `<script defer>` 연결
- [ ] `const`/`let`만 사용 (`var` 금지)
- [ ] `addEventListener`만 사용 (인라인 `onclick` 금지)
- [ ] `querySelector`/`querySelectorAll`, `textContent`/`innerHTML`, `classList` 조작
- [ ] `click`/`submit`/`scroll`/`input` 이벤트 처리, `preventDefault()`

### 인터랙션
- [ ] 햄버거 메뉴 토글
- [ ] 부드러운 스크롤 (앵커 클릭 시)
- [ ] 스크롤 탑 버튼 (300px 기준)
- [ ] 스크롤 시 네비 배경색 변경 (60px 기준)
- [ ] 다크모드 토글 + localStorage 유지
- [ ] Intersection Observer 스크롤 애니메이션 (threshold 0.2)
- [ ] Contact 폼 유효성 검사 (필수값, 이메일 형식, 에러 메시지, 성공 메시지)

### ES6+ 문법 & 배열 메서드
- [ ] 화살표 함수
- [ ] 템플릿 리터럴로 카드 HTML 생성
- [ ] 구조분해 할당
- [ ] `map`으로 GitHub 데이터 → 카드 변환
- [ ] `filter` (언어별 필터, 선택)
- [ ] `forEach` 배열 순회

### 비동기 처리 & API 연동
- [ ] `fetch` + `async/await`로 `https://api.github.com/users/{username}/repos` 호출
- [ ] 로딩 상태 UI (스피너/텍스트)
- [ ] 성공 상태 UI (카드 리스트)
- [ ] 에러 상태 UI ("프로젝트를 불러올 수 없습니다" + 재시도 버튼, 403 레이트리밋 대응)
- [ ] 빈 상태 UI ("표시할 프로젝트가 없습니다")
- [ ] `try/catch` 에러 처리

### 배포 & 문서화
- [ ] GitHub Pages 배포
- [ ] 배포 URL에서 전체 기능 정상 동작 확인
- [ ] README에 소개/기술스택/배포URL/스크린샷 포함
